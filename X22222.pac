// ════════════════════════════════════════════════════════════════
//  PROXY PAC — Jordan-Anchored Routing  v2
//  إضافات: Match Hunting + Auto-Reset بعد نهاية المباراة
// ════════════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════════════
//  ❺  RANGE LOOKUP — البحث في قاعدة النطاقات
// ════════════════════════════════════════════════════════════════
...
var SESSION = {
  ...
  phase:         "general",
  inMatch:       false,
  matchHunting:  false,   // ◄ وضع البحث عن سيرفر أردني
  huntingCount:  0,       // ◄ عدد طلبات Hunting منذ بدء البحث
  lastMatchMs:   0,       // ◄ وقت آخر حزمة match (لاكتشاف النهاية)
  lastActivity:  0
};

// ─── مدد زمنية ───────────────────────────────────────────────
var SESSION_TIMEOUT_MS  = 30 * 60 * 1000;   // 30 دقيقة بدون نشاط
var MATCH_END_TIMEOUT   = 45 * 1000;        // 45 ثانية بدون حزمة match = نهاية المباراة
var MAX_HUNTING         = 80;               // حد أقصى لطلبات Hunting (منع حلقات لانهائية)
...
// ════════════════════════════════════════════════════════════════
//  ❾  AUTO-RESET — إعادة تعيين الجلسة تلقائياً
//  يُشغَّل في حالتين:
//    أ) انقضاء SESSION_TIMEOUT بدون أي نشاط
//    ب) انقضاء MATCH_END_TIMEOUT بدون حزم match (= انتهت المباراة)
// ════════════════════════════════════════════════════════════════
...
function checkAndReset(now) {
  // إعادة تعيين كاملة بعد انتهاء وقت الخمول
  if (SESSION.lastActivity > 0 &&
      (now - SESSION.lastActivity) > SESSION_TIMEOUT_MS) {
    autoReset();
    return;
  }
  // ◄ الجديد: اكتشاف نهاية المباراة عبر غياب حزم match
  if (SESSION.inMatch && SESSION.lastMatchMs > 0 &&
      (now - SESSION.lastMatchMs) > MATCH_END_TIMEOUT) {
    autoReset();   // ← السكربت يعلم أن المباراة انتهت → صفّر
  }
}

// ════════════════════════════════════════════════════════════════
//  ❿  MATCH HUNTING — البحث عن سيرفر أردني
//
//  المشكلة السابقة:
//    إذا حلّ DNS على IP غير أردني → BLOCK فوري → المباراة تنتهي
//
//  الحل:
//    أثناء مرحلة match، أي IP تجاوز فحص BLOCK (أي ليس private)
//    يُمرَّر عبر PROXY بدلاً من حجبه، ويُحسَب ضمن hunting.
//    الـ Proxy هو من يقرر ما إذا كان الاتصال ممكناً.
//    بعد MAX_HUNTING محاولة بدون نطاق أردني → reset للمحاولة التالية.
// ════════════════════════════════════════════════════════════════
...
function handleMatchHunting() {
  ...
  // إذا تجاوزنا الحد الأقصى للمحاولات → reset ونبدأ دورة جديدة
  if (SESSION.huntingCount >= MAX_HUNTING) {
    autoReset();
  }
  // نُمرّر عبر الـ Proxy ليحاول الوصول (لا نحجب)
  return PROXY;
}
...
function FindProxyForURL(url, host) {
  ...
  // ── فحص Auto-Reset في كل طلب ──
  checkAndReset(now);
  SESSION.lastActivity = now;

  // ── تحديث مرحلة الجلسة مع اكتشاف انتهاء المباراة ──
  if (phase === "match") {
    SESSION.lastMatchMs  = now;
    SESSION.inMatch      = true;
    SESSION.phase        = "match";
    SESSION.matchHunting = false;   // سيُعاد تعيينه في حالة عدم وجود IP أردني
  } else if (SESSION.inMatch && phase !== "match") {
    // ◄ انتقلنا من match إلى lobby/general → المباراة انتهت → reset
    autoReset();
    SESSION.phase = phase;
  } else {
    SESSION.phase = phase;
  ...
  if (ip.indexOf(":") !== -1) {
    ...
    if (rid6 === -1) {
      // ◄ غير أردني — إذا كنا في مرحلة match → Hunting بدل Block
      if (phase === "match") return handleMatchHunting();
      return BLOCK;
    }

    // نطاق أردني مؤكد → ربط الجلسة + إلغاء Hunting
