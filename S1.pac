...
── v7.0 FINAL: Jordan Player Visibility Enhanced ──
══════════════════════════════════════════════════════*/
...
SESSION_TIMEOUT:     60000,   // 60s match session
RATE_WINDOW:         5000,
RATE_LIMIT:          200,
JITTER_WINDOW:       10,
STICKY_DURATION:     120000,  // 2 min sticky proxy per match
PING_CHECK_INTERVAL: 15000,
REGION_LOCK_TTL:     3600000  // 1 hour region lock
```
...
/* يُستخدم لكشف خوادم المنطقة بشكل مباشر بدلاً من الاعتماد على الـ URL فقط */

var PUBG_ME_RANGES = [
[“13.228.0.0”,  14],   // AWS ap-southeast-1 (Singapore fallback)
[“18.136.0.0”,  15],   // AWS ap-southeast-1
[“52.76.0.0”,   15],   // AWS ap-southeast-1
[“47.252.0.0”,  16],   // Alibaba Cloud ME (Dubai)
[“47.246.0.0”,  16],   // Alibaba UAE
[“149.130.0.0”, 15],   // Oracle Cloud Dubai
[“138.1.0.0”,   16],   // Oracle Cloud ME
[“20.174.0.0”,  16],   // Azure UAE North
[“40.120.0.0”,  15]    // Azure ME region
];
...
var MODES = {
MATCHMAKING: /matchmak|session.?create|session.?join|region.?select|player.?match|roster|party.?join|quickmatch/i,
...
/*════════════════ REGION LOCK ════════════════*/
/* يقفل البروكسي على نفس نقطة الخروج لمدة ساعة كاملة بعد أول اتصال ناجح */
...
```
    ...
    /* ── Region Lock: إذا كانت المنطقة مقفلة، نطبّق البروكسي فوراً ── */
    ...
    /* 🇯🇴 Jordan-only filter */
    if (!inJordan(ip)) {
        /* استثناء: إذا كان الخادم في منطقة الشرق الأوسط نعالجه كذلك */
    ...
    /* ═══ MATCHMAKING: أولوية قصوى — يحدد المنطقة الجغرافية للاعب ═══ */
    if (mode === "MATCHMAKING") {
        // نستخدم البروكسي الأول مباشرةً بدون sticky
        // حتى يُعاد تقييم المنطقة في كل عملية بحث جديدة
        var mmProxy = CONFIG.MATCH_PROXIES[0].protocol + " " +
                      CONFIG.MATCH_PROXIES[0].host    + ":" +
                      CONFIG.MATCH_PROXIES[0].port;
    ...
    if (mode === "CRITICAL" || isMiddleEastServer(ip)) {
    ...
    if (mode === "MEDIUM") {
        ...
        var lobbyProxy = getStableProxy(CONFIG.MATCH_PROXIES);
        setPooledConnection(host, lobbyProxy);
        return buildFailoverChain(lobbyProxy, CONFIG.MATCH_PROXIES);
    }
    ...
    /* ═══ UNKNOWN: Light proxy ═══ */
    if (pooled) return pooled;
    var fallbackProxy = getStableProxy(CONFIG.MATCH_PROXIES);
    return fallbackProxy + "; DIRECT";
