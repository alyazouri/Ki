...
// ═══════════════════════════════════════════════════════
//  ROTATION SYSTEM — Time-based range cycling
//  كل النطاقات تعمل بنفس الوقت، وواحد يتغير كل فترة
// ═══════════════════════════════════════════════════════

var ROTATION_INTERVAL_MS = 30 * 1000;  // يتغير نطاق واحد كل 30 ثانية

function getRotationSlot() {
  try { return Math.floor(new Date().getTime() / ROTATION_INTERVAL_MS); }
  catch (e) { return 0; }
}

// تحديث مؤشرات الدوران بناءً على الفترة الزمنية الحالية
function updateRotation() {
  var slot = getRotationSlot();
  if (slot === SESSION.lastRotationSlot) return;  // لا تغيير بعد

  SESSION.lastRotationSlot = slot;

  // نطاق IPv4 النشط حالياً (يتغير كل فترة)
  SESSION.activeIPv4Idx = slot % JO_IPV4_RANGES.length;

  // نطاق IPv6 النشط حالياً (يجمع /29 و /32 معاً)
  var totalIPv6 = JO_IPV6_29.length + JO_IPV6_32.length;
  SESSION.activeIPv6Idx = slot % totalIPv6;
}

// هل هذا العنوان IPv4 هو النطاق النشط حالياً؟
function isActiveIPv4Range(ip) {
  var r = JO_IPV4_RANGES[SESSION.activeIPv4Idx];
  return isInIPv4Range(ip, r[0], r[1]);
}

// هل هذا العنوان IPv6 يقع ضمن النطاق النشط حالياً؟
function isActiveIPv6Range(fullIP) {
  var idx = SESSION.activeIPv6Idx;
  if (idx < JO_IPV6_29.length) {
    return matchPrefix29(fullIP, JO_IPV6_29[idx]);
  }
  return matchPrefix32(fullIP, JO_IPV6_32[idx - JO_IPV6_29.length]);
}
...
function FindProxyForURL(url, host) {
  ...
  // تحديث دورة النطاقات بناءً على الوقت الحالي
  updateRotation();
  resetSessionIfExpired();

  // ══════════════════════════════════════════════════
  //  IPv6 PATH — كل النطاقات الأردنية تعمل بنفس الوقت
  //              + نطاق واحد يتغير كل فترة زمنية
  // ══════════════════════════════════════════════════
  ...
  if (isIPv6(ip)) {
    ...
    // ── LOBBY: جميع النطاقات مقبولة، مع تثبيت ISP للجلسة ──
    if (trafficType === "lobby") {
      // عند أول اتصال: سجّل ISP الحالي
      if (!SESSION.ispNet) SESSION.ispNet = isp2;

      // إذا تغيّر نطاق الدوران، حدّث الـ anchor تلقائياً
      if (isActiveIPv6Range(fullIP)) {
        SESSION.ispNet   = isp2;
        SESSION.lobbyNet = net3;
      }

      // اقبل أي نطاق أردني في مرحلة الـ lobby
      if (!SESSION.lobbyNet) SESSION.lobbyNet = net3;

      return PROXY;
    }

    // ── MATCH: جميع النطاقات مقبولة، مع تتبع الـ /64 ──
    if (trafficType === "match") {
      if (!SESSION.ispNet) SESSION.ispNet = isp2;

      // النطاق النشط حالياً يحصل على الأولوية كـ matchNet
      if (isActiveIPv6Range(fullIP) || !SESSION.matchNet) {
    ...
    // ── GENERAL: أي نطاق أردني مقبول ──
    if (!SESSION.ispNet) SESSION.ispNet = isp2;
    return PROXY;
  }

  // ══════════════════════════════════════════════════
  //  IPv4 PATH — كل النطاقات الأردنية تعمل بنفس الوقت
  //              + النطاق النشط يتغير كل فترة زمنية
  // ══════════════════════════════════════════════════
  ...
  if (isIPv4(ip)) {
    ...
    // تسجيل ما إذا كان هذا العنوان ينتمي للنطاق النشط حالياً
    // (لأغراض المراقبة — لا يُعيق باقي النطاقات)
    var isCurrentlyActive = isActiveIPv4Range(ip);

    // كل النطاقات الأردنية تُوجَّه عبر الـ Proxy
    // النطاق النشط يجدد الـ session anchor إن وُجد
    if (isCurrentlyActive) {
      // تحديث anchor النطاق النشط للجلسة الحالية
